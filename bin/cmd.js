...
var server = new Server({
    ...
    udp: argv.udp,
    ws: argv.ws,
    peersCacheLength: 800,
    peersCacheTtl: 20 * 60 * 1000 // 20 minutes
})
...
server.on('update', function(addr, params) {
    if (!argv.quiet) {
        ...
        console.log("update,infoHash:" + infoHash + ",toPeer:" + addr + ",peers:" + peers.length)
    }
})
...
server.on('start', function(addr, params) {
    if (!argv.quiet) {
        var infoHash = params.info_hash
        var peers = server.torrents[infoHash].peers
        console.log("start,infoHash:" + infoHash + ",toPeer:" + addr + ",peers:" + peers.length)
    }
})
server.on('stop', function(addr, params) {
    if (!argv.quiet) {
        var infoHash = params.info_hash
        var peers = server.torrents[infoHash].peers
        console.log("stop,infoHash:" + infoHash + ",toPeer:" + addr + ",peers:" + peers.length)
    }
})
